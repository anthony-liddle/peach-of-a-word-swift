...
struct StreakTests {
    ...
    func extendsAndBreaks() {
        storage.recordDailyCleared(dayIndex: 10, todayIndex: 10)
        #expect(storage.currentStreak(todayIndex: 10) == 1)
        storage.recordDailyCleared(dayIndex: 11, todayIndex: 11)
        #expect(storage.currentStreak(todayIndex: 11) == 2)
        // Skip day 12.
        storage.recordDailyCleared(dayIndex: 13, todayIndex: 13)
...
struct StorageEpochTests {
    ...
    func progressSurvivesEpochMove() {
        ...
        storage.recordDailyCleared(dayIndex: key, todayIndex: key)
...
struct AdoptStreakTests {
    ...
    func adoptsLiveHigher() {
        ...
        s.recordDailyCleared(dayIndex: today + 1, todayIndex: today + 1)
    ...
    func liveBeatsDeadStored() {
        let s = storage
        s.recordDailyCleared(dayIndex: today - 8, todayIndex: today - 8)
        for d in (today - 7)...(today - 5) { s.recordDailyCleared(dayIndex: d, todayIndex: d) }
    ...
    func rejectsSmaller() {
        let s = storage
        s.recordDailyCleared(dayIndex: today, todayIndex: today)
...
struct RolloverTests {
    ...
    func streakSurvives() {
        let s = storage
        s.recordDailyCleared(dayIndex: yesterday - 1, todayIndex: yesterday - 1)
        s.recordDailyCleared(dayIndex: yesterday, todayIndex: yesterday)
    ...
    func newDayCanStillBeRecorded() {
        let s = storage
        s.recordDailyCleared(dayIndex: yesterday, todayIndex: yesterday)
        s.recordDailyCleared(dayIndex: yesterday, todayIndex: yesterday)      // twice, still one day
