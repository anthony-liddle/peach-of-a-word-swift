...
Five, of which two were found *by porting* rather than by reading. None fixed, because
...
Not a bug, but the fossil of one. The warning survives into the Swift port because
...
packed binary blob searched in place, and **not** a database, which would add a
...
| **Out of scope, permanently** | n/a | The data pipeline stays in the web repo. The Swift side only ever consumes baked output. This is already how the port is structured. |
...
**Yes, but change what gets built next.** Not more engine, because the engine is done,
...
Note on style: this document contains no em dashes, per the build prompt. The
engine sections above were written before that instruction existed and did
carry them; they were retrofitted on 2026-08-10, along with the rest of the
repository. Predating a convention is not an exemption from it, and leaving
known instances is how a convention erodes. Only the punctuation changed. No
finding, number or judgement in this report was altered.
